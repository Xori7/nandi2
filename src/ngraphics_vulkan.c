...
void create_instance(n_graphics_t *context) {
...
void create_surface(n_graphics_t *context, NWindow window) {
#if PLATFORM == WINDOWS
...
void get_physical_devices(n_graphics_t *context) {
    uint32_t count;
...
void pick_physical_device(n_graphics_t *context) {
...
QueueFamilyIndices find_queue_families(n_graphics_t *context) {
...
void create_logical_device(n_graphics_t *context) {
...
void create_swap_chain(n_graphics_t *context, NWindow window) {
...
VkImageView
create_image_view(n_graphics_t *context, VkImage image, VkFormat format, VkImageAspectFlags aspectFlags) {
...
void create_image_views(n_graphics_t *context) {
...
VkShaderModule create_shader_module(n_graphics_t *context, const char *code, uint32_t size) {
...
VkFormat find_depth_format(n_graphics_t *context);
void create_render_pass(n_graphics_t *context) {
...
void create_graphics_pipeline(n_graphics_t *context, NMaterial *material) {
...
void create_frame_buffers(n_graphics_t *context) {
...
void create_command_pool(n_graphics_t *context) {
...
VkFormat find_supported_depth_format(n_graphics_t *context, NList_VkFormat candidates, VkImageTiling tiling, VkFormatFeatureFlags features) {
...
void create_image(n_graphics_t *context, uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling,
                  VkImageUsageFlags usage,
                  VkMemoryPropertyFlags properties, VkImage *image, VkDeviceMemory *imageMemory);

void create_depth_resources(n_graphics_t *context) {
...
uint32_t find_memory_type(n_graphics_t *context, uint32_t typeFilter, VkMemoryPropertyFlags properties) {
...
void
create_buffer(n_graphics_t *context, VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties,
              VkBuffer *buffer,
...
VkCommandBuffer begin_single_time_commands(n_graphics_t *context) {
...
void end_single_time_commands(n_graphics_t *context, VkCommandBuffer commandBuffer) {
...
void copy_buffer(n_graphics_t *context, VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size) {
...
void update_vertex_buffer(n_graphics_t *context, NMesh *mesh) {
...
void create_vertex_buffer(n_graphics_t *context, NMesh *mesh) {
    VkDeviceSize bufferSize = mesh->material->vertexDescriptor.size * mesh->vertices.count;
...
void update_index_buffer(n_graphics_t *context, NMesh *mesh) {
...
void create_index_buffer(n_graphics_t *context, NMesh *mesh) {
    VkDeviceSize bufferSize = sizeof(uint32_t) * mesh->indices.count;
...
void create_uniform_buffers(n_graphics_t *context, NMaterial *material) {
...
void create_descriptor_pool(n_graphics_t *context, NMaterial *material) {
...
void create_descriptor_sets(n_graphics_t *context, NMaterial *material) {
...
void create_descriptor_set_layout(n_graphics_t *context, NMaterial *material) {
...
void create_command_buffers(n_graphics_t *context) {
...
void record_command_buffer(n_graphics_t *context, VkCommandBuffer commandBuffer, uint32_t imageIndex) {
    VkCommandBufferBeginInfo beginInfo = {
            .sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO,
            .flags = 0,
...
void create_sync_objects(n_graphics_t *context) {
...
void update_camera_matrix(n_graphics_t *context) {
    NMatrix4x4 view;
...
void cleanup_swap_chain(n_graphics_t *context) {
...
extern void n_graphics_recreate_swap_chain(n_graphics_t *context, NWindow window) {
    vkDeviceWaitIdle(context->device);
...
extern n_graphics_t n_graphics_initialize(NLogger logger, NWindow window) {
    n_graphics_t context = {
            .logger = logger
    };
...
void transition_image_layout(n_graphics_t *context, VkImage image, VkFormat format, VkImageLayout oldLayout,
...
void copy_buffer_to_image(n_graphics_t *context, VkBuffer buffer, VkImage image, uint32_t width, uint32_t height) {
    VkCommandBuffer commandBuffer = begin_single_time_commands(context);
...
VkSampler create_texture_sampler(n_graphics_t *context) {
...
extern NTexture n_graphics_texture_create(n_graphics_t *context, const char *path) {
    NTexture texture = {0};
...
extern void n_graphics_texture_destroy(n_graphics_t *context, NTexture texture) {
